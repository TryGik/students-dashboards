...
function createTableElements(fullName, faculty, birthDate, educationPeriod) {
    ...
    birthDateDiv.textContent = birthDate;
...
btnAddStudent.addEventListener('click', (e) => {
    if (validation.isValid) {
        ...
        studentInfo.birthDate = birthDateItem.patternDate;
        studentInfo.educationPeriod = educationPeriodItem;
        students.push(studentInfo);
        localStorage.setItem('students', JSON.stringify(students));

        createTableElements(fullNameItem, facultyItem, birthDateItem.patternDate, educationPeriodItem);
...
sortFullName.addEventListener('click', () => {
    ...
    clearField();
    localStorage.clear();
    sortStudent(newStudentList, 'fullName');
...
sortFaculty.addEventListener('click', () => {
    ...
    clearField();
    localStorage.clear();
    sortStudent(newStudentList, 'faculty');
    localStorage.setItem('students', JSON.stringify(newStudentList));
    showElements();    
});
sortDataBirth.addEventListener('click', () => {
    ...
    clearField();
    localStorage.clear();
    sortStudent(newStudentList, 'birthDate')
    localStorage.setItem('students', JSON.stringify(newStudentList));
    showElements()    
});
sortDataEdu.addEventListener('click', () => {
    ...
    newStudentList.sort((a,b) => {
        if (a.educationPeriod.patternDate > b.educationPeriod.patternDate) {
            return 1;
        }
        if (a.educationPeriod.patternDate < b.educationPeriod.patternDate) {
            return -1;
...
function SearchSortSubStr(students, key) {
...
searchBtn.addEventListener('click', (e) => {
    ...
    if (result > 0 && searchInput.value !== '') {
        localStorage.setItem('students', JSON.stringify(SearchSortSubStr(newStudentList, searchInput.value)));
        SearchSortSubStr(newStudentList, searchInput.value).forEach(item => {
            createTableElements(item.fullName, item.faculty, item.birthDate, item.educationPeriod);
        })
